...
export class DirectorView extends React.Component {
  ...
  getUserData(token) {
    axios
      .get(
        `https://myflickz.onrender.com/users/${localStorage.getItem("user")}`,
        {
    ...
    return (
      <Container className="director-view">
        <a onClick={this.goBack}>
          <FontAwesomeIcon
            icon={faChevronLeft}
            className="mr-2 mr-sm-4 go-back"
          />
        </a>
        ...
        <Media className="d-flex flex-lg-row flex-xs-column flex-sm-column">
          <Media.Body>
            <Link to={`/`}>
              <Button
                className="director-movies-button"
                size="sm"
            ...
            {director.hasOwnProperty("Death") ? (
              <div className="director-dateofDeath">
                <span className="value">Died in {director.Death}</span>
              </div>
            ) : (
              ""
            )}
        ...
        <div className="director-view">
          <br />
          <h4 className=" some-movies-director mt-4 ml-30">
            Some movies from {director.Name}
          </h4>
